...
1. If `ToolSearch` returns zero matches: `Bash("sleep 2")`, then retry `ToolSearch`. Do this up to **10 times** (~20s total wait). Only after the 10th failure should you emit a `scribe_offline` warning.
2. If `bujo_read` errors with a non-`InputValidationError` (connection/transport error): `Bash("sleep 2")`, then retry `bujo_read`. Up to **10 retries** before concluding offline.
3. If `bujo_read` errors with `InputValidationError`: re-run the ToolSearch — the schema was cleared. No sleep needed.

The 20s budget covers cold-start scenarios — `uv run` resolving the lockfile + Python interpreter + module imports + first AppleScript call to a Notes app that's still booting itself. A genuine outage (scribe binary missing, `pyproject.toml` broken, etc.) won't be fixed by waiting longer than this; if it's not up by 20s, it's not coming up.

Only emit `scribe_offline` in `warnings` after exhausting retries on a genuine transport error. A schema miss is never offline; a zero-match ToolSearch during the first 20s is never offline.
